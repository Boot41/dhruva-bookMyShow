...
router = APIRouter(tags=["theater_admins"])  # Theater admin membership management
...
def create_membership(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=400, detail="Membership already exists for this user and theater or invalid references")  # Maps DB unique/FK errors to 400
...
def list_memberships(
    user_id: Optional[int] = Query(None, description="Filter by user_id"),  # Optional filters shape the query
...
def get_membership(membership_id: int, db: Session = Depends(get_db)):
    ...
    if not membership:
        raise HTTPException(status_code=404, detail="Membership not found")  # 404 when resource absent
    return membership
